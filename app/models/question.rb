class Question < ApplicationRecord
  ...
  # You should also add a dependent option. The values can be:
  ...
  # this validates that the title/body combination is unique which means that title doesn't have to be unique by itself, body doesn't have to be unique by itself but the combination of the 2 must be unique
  # validates :body, uniqueness: {scope: :title}

  # VALID_EMAIL_REGEX = /\A([\w+\-]\.?)+@[a-z\d\-]+(\.[a-z]+)*\.[a-z]+\z/i
  # validates :email, format: VALID_EMAIL_REGEX

  # this defines a custom validation. it takes a first argument which in this case is a private method
